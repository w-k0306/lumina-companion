...
personaRouter.post("/analyze", async (req, res) => {
  try {
    ...
    const analysis = await generatePersonaFromChat(chatText, {
      name: name || "朋友 Bot", relation: relation || "朋友", similarity: similarity || 2
    });
    savePersona(personaId, { id: personaId, name: name || "朋友 Bot", relation: relation || "朋友", ...analysis, createdAt: Date.now() });
...
personaRouter.post("/quick", async (req, res) => {
  try {
    ...
    const personaId = uuidv4();
    const styles = {
      playful: { desc: "聊天节奏偏快，喜欢接梗和轻微吐槽，但熟了之后会认真给建议。", tags: ["笑点很低","先吐槽再认真","会接梗","回复很快"], systemPrompt: "你是一个说话轻松幽默的朋友，喜欢用短句，会吐槽但不伤人，偶尔用emoji。" },
      calm: { desc: "语气更柔和，常先接住情绪，再慢慢追问事情的细节。", tags: ["温柔安慰","慢慢听你说","记得细节","少量反问"], systemPrompt: "你是一个温柔体贴的朋友，善于倾听，会记住对方说过的细节，回复不急不躁。" },
      direct: { desc: "语气自然直接，常用短句确认对方状态，再给出熟人式回应。", tags: ["自然短句","熟人感","轻松回应","会追问"], systemPrompt: "你是一个说话直接的朋友，不绕弯子，但关心对方，会追问细节。" }
    };
    const profile = styles[style] || styles.direct;
    savePersona(personaId, { id: personaId, name: name || "Lumina", relation: "朋友", ...profile, quote: "", createdAt: Date.now() });
