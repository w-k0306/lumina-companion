...
async function generatePersonaFromChat(chatText, options) {
  ...
  const response = await fetch(AI_API_URL + "/chat/completions", {
    ...
    body: JSON.stringify({ model: AI_MODEL, messages: [{ role: "system", content: "只返回JSON，不要其他内容。" }, { role: "user", content: prompt }], temperature: 0.3, max_tokens: 1000 })
  });
  ...
  try {
    ...
    return { desc: "语气自然直接，常用短句，熟人式回应。", tags: ["自然短句","熟人感","轻松回应","会追问"], quote: "你继续说，我听着呢。", systemPrompt: "你是一个说话自然的朋友，语气直接但不冷漠。" };
  }
}
